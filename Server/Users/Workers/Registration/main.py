...
from Server.Users.Workers.Registration.CommandProcessor import CommandProcessor
from Server.Users.Workers.Registration.Config import amqp as amqp_listener_config
...
def spawn_worker():
    ...
    registration_events_listener = amqp_listener(amqp_listener_config.LISTEN_FROM_PARAMETERS,
                                                 callback=command_processor.do_command)
    registration_events_listener_thread = threading.Thread(target=registration_events_listener.run)
    registration_events_listener_thread.start()
